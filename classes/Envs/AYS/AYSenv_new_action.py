...
class AYS_Environment_new_action(Env):
    """
    ...
    management_cost = 0.1
    action_space = [(False, False, False), (True, False, False), (False, True, False), (True, True, False),
    ...
    def step(self, action: int):
        ...
        reward = self.reward_function(
            action)  # TODO check if this might be needed before step is done to evaluate the current state, not the next state!
    ...
    def _get_parameters(self, action=0):
        ...
        parameter_list = [(self.beta_LG if action_tuple[0] else self.beta,
                           ...
                           self.tau_CC if action_tuple[2] else self.tau_A,
                           self.tau_S, self.theta)]
